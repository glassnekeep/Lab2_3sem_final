...
{   
    ...
    if (gc && values.size() > 0)
    {
        ...
        int segmentCount = std::get<0>(calculateChartSegmentCountAndRange(lowValue, highValue));
        double rangeLow = std::get<1>(calculateChartSegmentCountAndRange(lowValue, highValue));
        double rangeHigh = std::get<2>(calculateChartSegmentCountAndRange(lowValue, highValue));
        ...
        int iddd = evt.GetId();
        if(evt.GetId() == 121)
            gc->SetPen(wxPen(wxColor(255, 51, 255), 2));
        else if(evt.GetId() == 212)
            gc->SetPen(wxPen(wxColor(255, 255, 51), 2));
        else
            gc->SetPen(wxPen(wxColor(0, 255, 255), 2));
